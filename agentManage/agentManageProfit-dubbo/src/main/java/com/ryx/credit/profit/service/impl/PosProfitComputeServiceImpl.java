...
//                this.rewardAssessment(detail,currentDate);
    ...
    private void rewardAssessment(PosRewardDetail detail, String currentDate) {
        ...
        if(month.size() > 0){
            ...
            if(tranSum.compareTo(posRewardTemp.getGrowAmt().multiply(new BigDecimal("10000"))) >= 0){
                LOG.info("代理商唯一码：{}，代理商季度交易量达到活动指标，不进行扣减奖励", detail.getPosAgentId());
                return;
            }
            LOG.info("代理商唯一码：{}，总交易额：{}，活动设定总交易额：{}万，代理商季度交易量指标不合格，进行扣减奖励", detail.getPosAgentId(), posRewardTemp.getGrowAmt());
    ...
    private void deductParentAgentPosRewardAmt(PosRewardDetail parentPosRewardDetail, BigDecimal profitAmt, String computType) {
        ...
        BigDecimal posRewardAmt = praentPosRewardAmt.subtract(profitAmt);
...
//        List<String> list = PosProfitComputeServiceImpl.getMonthBetween("2018-06", "2018-09");
//        list.forEach(s -> {
