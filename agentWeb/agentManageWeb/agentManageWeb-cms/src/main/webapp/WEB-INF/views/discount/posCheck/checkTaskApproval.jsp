...
<script>
    function submitApproval() {
        ...
        parent.$.messager.confirm('询问', '确认完成任务？', function(b) {
            if (b) {
                $.ajaxL({
                    type: "POST",
                    url: "/checkActivity/taskApproval",
                    dataType:'json',
    ...
    /*//驳回修改信息
    function editPosCheck(){
        parent.$.messager.confirm('询问', '您确定申请信息填写完毕吗？', function(b) {
            if (b) {
                $.ajaxL({
                    type: "POST",
                    url: "/discount/editCheckRegect",
                    dataType:'json',
                    traditional:true,
                    contentType:"application/x-www-form-urlencoded",
                    data: $('#posCheckEditFrom').serializeArray(),
                    beforeSend : function() {
                        progressLoad();
                    },
...
<div class="easyui-tabs" id="taskAppTabs">
    <div title="待审信息">
        <!--待审信息-->
        <shiro:hasPermission name="/thaw/checkApprovalQuery">
            <%@ include file="posCheckInfo.jsp" %>
        </shiro:hasPermission>

        <%--<shiro:hasPermission name="/thaw/checkApprovalUpdate">
            <%@ include file="posCheckEdit.jsp" %>
        </shiro:hasPermission>--%>
        <%--审批意见--%>
        <%@ include file="checkApproval_opinion.jsp" %>
        <%--<shiro:hasPermission name="/thaw/checkApprovalUpdate">
            <a href="javascript:void(0)" class="easyui-linkbutton" onclick="editPosCheck()" >保存</a>
        </shiro:hasPermission>--%>
