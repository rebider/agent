 ...
 * @Date 2019/2/18
 */
public class AgentOweTicketListener implements TaskListener, ExecutionListener {
    ...
    public void notify(DelegateExecution delegateExecution) throws Exception {
        ...
        if ("start".equals(eventName)) {
            ...
            //数据变更服务类
            IAgeInvoiceApplyService ageInvoiceApplyService = (IAgeInvoiceApplyService) MySpringContextHandler.applicationContext.getBean("ageInvoiceApplyService");
            //审批拒绝
            if ("reject_end".equals(activityName)) {
                logger.info("reject========");
                ageInvoiceApplyService.completeTaskEnterActivity(delegateExecution.getProcessInstanceId(), "2");
            }
            //审批同意更新数据库
            if ("finish_end".equals(activityName)) {
                ageInvoiceApplyService.completeTaskEnterActivity(delegateExecution.getProcessInstanceId(), "1");
