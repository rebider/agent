...
public class ExcelExportSXXSSF {
	...
	public void writeDatasByMap(List<Map<String,Object>> datalist) throws Exception {
		...
		for (int j = 0; j < datalist.size(); j++) {
			...
			for (int cellnum = 0; cellnum < fieldCodes.size(); cellnum++) {
				Object owner = datalist.get(j);
				Object value = invokeMapMethod(owner, fieldCodes.get(cellnum),
						new Object[] {});
				Cell cell = row.createCell(cellnum);
				cell.setCellValue(value!=null?value.toString():"");
			}
	...
	private Object invokeMapMethod(Object owner, String fieldname, Object[] args)
			throws Exception {
		...
		return method.invoke(owner, fieldname);
	}
