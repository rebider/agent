...
public class ProfitComputerServiceImpl implements ProfitComputerService {
     ...
     * @param computType
     * @Author: Zhang Lei
     * @Description: 直发平台补款（从上往下）
     * @Date: 20:12 2019/3/19
     */
    @Override
    public void computer_Supply_ZhiFa(String profitMonth, String computType) {
        List<ProfitDirect> profitDirects = directMapper.selectBySupply();//存在补款的记录，追溯上级补款
        String month = profitMonth;
        for (ProfitDirect profitDirect : profitDirects) {
            ...
            supplyWhere.setBusBigType("02");
            BigDecimal supply = profitSupplyMapper.getBuckleByMonthAndPid(supplyWhere);//当前代理商总补款金额
            if (null == supply || supply.compareTo(BigDecimal.ZERO) == 0) {
                //无补款
                continue;
            }
            BigDecimal creditAmt = buckleRunMapper.getSumRunAmt(profitDirect.getAgentId());//一共被代扣总额，就是总欠别人多少钱
            if (null != creditAmt && creditAmt.compareTo(BigDecimal.ZERO) > 0) {
                ...
                for (BuckleRun buckleRun : buckleRuns) {
                    ...
                    BigDecimal c = a.subtract(b);//还欠上级多少钱
                    if (c.compareTo(BigDecimal.ZERO) == 0) {
                        continue;
                    }

                    BigDecimal nowbk = BigDecimal.ZERO;
                    if(supply.compareTo(c)>=0){
                        nowbk = c;
                        supply = supply.subtract(nowbk);//剩余可补金额
                    }else {
                        nowbk = supply;
                    ...
                    ProfitDirect bearProfitDirect = directMapper.selectByAgentAndMon(record);

                    if (bearProfitDirect.getParentAgentId().equals(bearProfitDirect.getFristAgentId())) {//扣款历史为一代，更新一代补款
                        AgentBusInfo agentBusInfo = businfoService.getByBusidAndCode("6000",bearProfitDirect.getAgentId());
                        ProfitDetailMonth detailMonth = detailMonthMapper.selectByAgentId(agentBusInfo.getAgentId());
                        BigDecimal s = detailMonth.getZhifaSupply() == null ? BigDecimal.ZERO : detailMonth.getZhifaSupply();
                        detailMonth.setZhifaSupply(s.add(nowbk));
                        detailMonthMapper.updateByPrimaryKeySelective(detailMonth);

                    } else {//扣款历史为非一代，更新上级补款
                        BigDecimal s = bearProfitDirect.getSupplyAmt() == null ? BigDecimal.ZERO : bearProfitDirect.getSupplyAmt();
                        bearProfitDirect.setSupplyAmt(s.add(c));

                        BigDecimal ps = bearProfitDirect.getParentSupply() == null ? BigDecimal.ZERO : bearProfitDirect.getParentSupply();
                        bearProfitDirect.setParentSupply(ps.add(c));
                        directMapper.updateByPrimaryKeySelective(bearProfitDirect);
                    }
                ...
                //剩余的补给自己
                BigDecimal d = profitDirect.getSupplyAmt() == null ? BigDecimal.ZERO : profitDirect.getSupplyAmt();
                profitDirect.setSupplyAmt(d.add(supply));
    ...
    public void computerSurplus(ProfitDirect profitDirect, int index, String oldAgrnt, BigDecimal buck, String computType) {
        ...
        if (profitDirect.getFristAgentId().equals(profitDirect.getParentAgentId())) { //上级就是一级代理商
            //帮下级承担扣款
            AgentBusInfo agentBusInfo = businfoService.getByBusidAndCode("6000",parentDirect.getAgentId());
            ProfitDetailMonth detailMonth = detailMonthMapper.selectByAgentId(agentBusInfo.getAgentId());//一级代理商信息
...
//        BigDecimal total = tranAmount.add(zfAmount);
    ...
    @Override
    public ProfitDetailMonth getTaxAndProfit(BigDecimal profitA, String agentPid, BigDecimal subAmt, BigDecimal
            agentTax, String transDate, boolean isJieYin, boolean isRYX) {
...
//            }
            ...
            BigDecimal smalTax = profitDetail.getSmalTaxAmt() == null ? BigDecimal.ZERO : profitDetail.getSmalTaxAmt();
            ;//已抵税金额
     ...
     */
    public ProfitDetailMonth getNewTaxAndProfit(ProfitDetailMonth detailMonth, BigDecimal subAmt, String
            transDate, BigDecimal dayTotal, boolean isRYX, boolean isSupply) {
        ...
        if (null == tax) {
            ...
            if (null == point) {
                ...
                tax = point.getCloTaxPoint() == null ? new BigDecimal("0.06") : point.getCloTaxPoint();//税点为空默认0.06
        }
