    ...
    <form id="agentBasics_Edit">
        <table class="grid">
            ...
            <tr>
                ...
                <td>营业执照到期日</td>
                <td><input name="agBusLice" id="agBusLice" type="text"  class="easyui-datebox" editable="false" placeholder="请输入" <%-- value="${agent.agBusLice}"--%> value=" <fmt:formatDate pattern="yyyy-MM-dd" value="${agent.agBusLice}" />"  style="width:120px;"  <shiro:lacksPermission name="/agentEnter/agentEdit/baseInfo">readonly="readonly"</shiro:lacksPermission>/>
            ...
            <tr>
                ...
                <td>
                    ...
                    <shiro:hasPermission name="/agentEnter/agentEdit/baseInfo">
                        <a href="javascript:void(0);" onclick="showRegionFrame({target:this,callBack:dqReturnRegion},'/region/departmentTree',false)">选择</a>
                    </shiro:hasPermission>
                    <a href="javascript:void(0);" onclick="del(this)">清除</a>
                </td>
                <td>业务对接省区</td>
...
<script type="text/javascript">
    ...
    function setAgBusLice() {
        $("#agentBasics_Edit #agBusLice").datebox("setValue", "2099-12-31");
    }
    //获取form数据
    function get_editAgentBasics_FormData(){
        return baseData = $.serializeObject($("#agentBasics_Edit"));
    }
    //地区选择
    ...
    //地区选择
    function dqReturnRegion(data,options){
        if (data.pid!='0'){
            info("请选择对应大区");
            $(options.target).parent().parent().find("#agDocPro1").val("");
            $(options.target).parent().parent().find("input[name='agDocPro']").val("");
            console.info(options.target);
            $(options.target).prev("input").val("");
            $(options.target).prev("input").prev("input").val("");
            return ;
        }
        var agDocPro_=$(options.target).prev("input").val();
        if (data.id!= agDocPro_){   //大区更改了
            $(options.target).parent().parent().find("#agDocPro1").val("");
            $(options.target).parent().parent().find("input[name='agDocPro']").val("");
        }
    ...
    function basicsVerifyIntegrity(){
        ...
        var table = $('#agentBasics_Edit .grid');
