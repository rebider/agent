...
<script type="text/javascript">
    var transFlowDataGrid;
   	var transAllocDataGrid;
    $('#allocdiv').click(function(){
    	var rows = transFlowDataGrid.datagrid('getSelected');
    	//i = rows.length-1;
    	//loanId = rows[i].loanId;
    	flowId = rows.flowId;
    	//console.info(rows[0]);
        transAllocDataGrid.datagrid('load', {
            id: flowId
        });
        transFlowDataGrid.datagrid('unselectAll').datagrid('uncheckAll');
    });
    $(function() {
        transFlowDataGrid = $('#transFlowDataGrid').datagrid({
            url : '${path }/transFlow/orderdataGrid',
            fit : true,
            striped : true,
            rownumbers : true,
            pagination : true,
            ...
            },  
            {
                ...
                formatter : function(value, row, index) {
                    var str = '';
                    <shiro:hasPermission name="/transFlow/editOrder">
                    str += $.formatString('<a href="javascript:void(0)" class="order-easyui-linkbutton-edit" data-options="plain:true,iconCls:\'fi-magnifying-glass icon-blue\'" onclick="queryOrderTransFlowFun(\'{0}\');" >查看</a>', row.flowId);
                   </shiro:hasPermission>
                    return str;
                } 
               }
       ...
       onLoadSuccess:function(data){
        ...
        transAllocDataGrid = $('#transAllocDataGrid').datagrid({
        url : '${path }/transFlow/transAllocDataGrid',
        fit : true,
        striped : true,
        rownumbers : true,
        pagination : true,
    ...
    //订单交易查看
    function queryOrderTransFlowFun(flowId) {
        parent.$.modalDialog({
            title : '详情',
            width : 1200,
            height : 420,
            href : '${path }/transFlow/editOrderTransFlowPage?flowId=' + flowId
        });
	}
    
    //还款分配查看
    function querycTransAlloc(flowId) {
        parent.$.modalDialog({
            title : '详情',
            width : 1000,
            height : 250,
            href : '${path }/transFlow/querycTransAlloc?flowId=' + flowId  
        });
	}
    
    function searchTransFlow() {
        transFlowDataGrid.datagrid('load', $.serializeObject($('#searchTransFlowForm')));
    }
    function cleanTransFlow() {
        $('#searchTransFlowForm input').val('');
        transFlowDataGrid.datagrid('load', {});
    }
    function exportUserFun(){
    	$.ajax({
    		url:'${path }/transFlow/exportUser',
    		data:$("#searchTransFlowForm").serialize(),
    		type:'GET',
    		dateType:'json',
    		success:function(data){
   				parent.$.messager.alert('提示', "导出成功", 'info');
    		}
    	})
    }
	
    $('#productrate').combotree({
...
<div class="easyui-layout" data-options="fit:true,border:false">
    <div data-options="region:'north',border:false" style="height: 110px; overflow: hidden;background-color: #fff">
        <form id="searchTransFlowForm" action="${path }/transFlow/exportUser" method="post">
			<table>
				<tr>
					<th>合同编号:</th>
                ...
                <tr>
					...
					<th>子产品名称:</th>
                    <td colspan="3"><select id="productrate" name="subProductId" class="easyui-combobox" data-options="width:140,height:20,editable:false,panelHeight:'auto'"></select>
                    <td>
				...
				<tr>
					<td>
						<button type="submit">导出订单流水</button>
					</td>
				</tr>
			</table>
		</form>
    ...
    <div  data-options="region:'center',border:true,title:'还款分配明细列表'" >
        <table id="transAllocDataGrid" data-options="fit:true,border:false"></table>
    </div> 
    
     <div id="allocdiv" data-options="region:'west',border:true,title:'订单信息查询'"  style="width:48%;overflow: hidden; ">
        <table id="transFlowDataGrid" data-options="fit:true,border:false"></table>
    </div>
    
    <div id="userToolbar" style="display: none;">
   		<!-- <a onclick="exportUserFun();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'fi-upload icon-green'">导出订单流水</a> -->
	</div>
</div>
